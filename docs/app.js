...
function sample(items, count) {
  if (items.length < count) {
    throw new Error(`Not enough questions in pool: requested ${count}, got ${items.length}.`);
  }
...
function questionImage(question) {
  const image = (question.images || []).find((entry) => entry.file && state.imageUrls.has(entry.file));
  if (!image) return "";
  const src = state.imageUrls.get(image.file);
  return `<img class="question-image" src="${src}" alt="" loading="lazy">`;
}
...
function renderAnswerList(question, { interactive = false, reveal = false, selected = null } = {}) {
  return (question.answers || []).map((answer, index) => {
    ...
    return `
      <label class="${classNames}">
        ${interactive ? `<input type="radio" name="answer" value="${index}" ${checked}>` : ""}
...
function renderStudy() {
  ...
  $("#studyCard").innerHTML = `
    ...
    <h3>${escapeHtml(question.question_text || question.image_text || "Frage ohne Text")}</h3>
    <div class="answers">${renderAnswerList(question, { interactive: true })}</div>
    <div id="studyExplanation" class="explanation" hidden>
      <strong>Richtige Antwort:</strong>
      ${correctAnswers(question).map((answer) => escapeHtml(answer.text)).join(" · ") || "Keine Antwort markiert."}
    </div>
  `;

  const inputs = $("#studyCard").querySelectorAll('input[name="answer"]');
  inputs.forEach((input) => {
    input.addEventListener("change", () => {
      const selected = Number(input.value);
      $("#studyCard").querySelectorAll(".answer").forEach((label, index) => {
        label.classList.toggle("correct", Boolean(question.answers[index]?.correct));
        label.classList.toggle("wrong", index === selected && !question.answers[index]?.correct);
      });
  ...
  $("#studyNext").textContent = state.studyIndex === total - 1 ? "Fertig ✓" : "Weiter →";
}

function startStudy(reset = true) {
  if (reset || !state.studyQuestions.length) {
    state.studyQuestions = [
      ...state.allQuestions.filter((q) => q.scope === "general").sort((a, b) => a.number - b.number),
      ...state.allQuestions
        .filter((q) => q.scope === "state" && q.state === state.selectedState)
        .sort((a, b) => a.number - b.number),
    ];
    if (state.studyQuestions.length !== 310) {
      throw new Error(`Expected 310 study questions for ${state.selectedState}, got ${state.studyQuestions.length}.`);
    }
    state.studyIndex = 0;
  }
  showView("study");
...
function buildExam() {
  const general = state.allQuestions.filter((q) => q.scope === "general");
  const stateQuestions = state.allQuestions.filter(
    (q) => q.scope === "state" && q.state === state.selectedState,
  );

  const poolA = general.filter((q) => q.number >= 1 && q.number <= 150);
  const poolB = general.filter((q) => q.number >= 151 && q.number <= 240);
  const poolC = general.filter((q) => q.number >= 241 && q.number <= 300);

  if (poolA.length < 10 || poolB.length < 10 || poolC.length < 10 || stateQuestions.length < 3) {
    throw new Error("The selected data set does not contain enough questions for the 33-question exam.");
  }
...
function renderExam() {
  $("#examProgress").innerHTML = progressBar(0, state.examQuestions.length);

  $("#examForm").innerHTML = state.examQuestions.map((question, index) => `
        ...
        ${(question.answers || []).map((answer, answerIndex) => `
          <label class="answer">
            <input
              ...
              name="exam-${index}"
              value="${answerIndex}"
              ${state.examAnswers[index] === answerIndex ? "checked" : ""}
            >
  ...
  $("#examForm").querySelectorAll("input[type=radio]").forEach((input) => {
    input.addEventListener("change", () => {
      const questionIndex = Number(input.name.replace("exam-", ""));
...
function updateTimer() {
  ...
  const remaining = Math.max(0, EXAM_TIME_SECONDS - elapsed);
...
function startExam() {
  ...
  state.examTimerId = setInterval(updateTimer, 1000);
  updateTimer();
  showView("exam");
}
...
function submitExam(autoSubmitted = false) {
  ...
  let score = 0;
  const details = state.examQuestions.map((question, index) => {
    ...
    const isCorrect = correctIndexes.includes(state.examAnswers[index]);
    if (isCorrect) score += 1;
    return { question, index, isCorrect, correct, selected: state.examAnswers[index] };
  });

  const passed = score >= PASSING_SCORE;
  $("#resultScore").textContent = `${score} / 33`;
  $("#resultTitle").textContent = passed ? "Bestanden" : "Noch nicht bestanden";
  ...
  $("#resultDetails").innerHTML = details.map((item) => {
    const selectedText = item.selected == null
      ...
      : item.question.answers[item.selected]?.text || "Unbekannte Antwort.";
    const correctText = item.correct.map((answer) => answer.text).join(" · ") || "Keine richtige Antwort hinterlegt.";

    return `
      <div class="result-item ${item.isCorrect ? "correct" : "wrong"}">
        <div class="result-index">${item.index + 1}</div>
        <div>
          <p><strong>${escapeHtml(item.question.question_text || item.question.image_text || "Frage")}</strong></p>
          <div class="result-answer">
            Deine Antwort: ${escapeHtml(selectedText)}<br>
            Richtig: ${escapeHtml(correctText)}
          </div>
        </div>
      </div>
    `;
...
async function loadLatestRelease() {
  ...
  if (!repository) {
    throw new Error("Could not determine the GitHub repository from the Pages URL.");
  }
  ...
  const jsonAsset = release.assets.find((asset) => asset.name === DATA_FILENAME);
  const imagesAsset = release.assets.find((asset) => asset.name === IMAGES_FILENAME);

  if (!jsonAsset || !imagesAsset) {
    throw new Error("The latest release is missing the JSON or illustration archive.");
  }

  const [questionsResponse, imagesResponse] = await Promise.all([
    fetch(jsonAsset.browser_download_url),
    fetch(imagesAsset.browser_download_url),
  ]);

  if (!questionsResponse.ok || !imagesResponse.ok) {
    throw new Error("One or more release assets could not be downloaded.");
  }

  const [questions, imageBytes] = await Promise.all([
    questionsResponse.json(),
    imagesResponse.arrayBuffer(),
  ]);

  return { release, questions, imageBytes };
}
...
function populateStates() {
  const names = [...new Set(
    state.allQuestions
      .filter((question) => question.scope === "state" && question.state)
...
async function init() {
  try {
    ...
    state.release = loaded.release;
    state.allQuestions = loaded.questions;
    loadImages(loaded.imageBytes);
    populateStates();

    const published = loaded.release.published_at
      ? new Date(loaded.release.published_at).toLocaleDateString("de-DE")
      : "";
    $("#releaseBadge").textContent = `Datenstand ${published || loaded.release.tag_name}`;
...
document.addEventListener("click", (event) => {
  ...
  try {
    if (action === "home") {
      ...
      startStudy(true);
    } else if (action === "study-reset") {
      startStudy(true);
    } else if (action === "exam") {
      startExam();
    } else if (action === "exam-again") {
      startExam();
    }
  } catch (error) {
    console.error(error);
